...
[Input & Guidance]
  ...
  • LoadImage (style reference) → Ollama advisor + IPAdapter branch
  • String (user prompt stub)
  • OllamaParameterAdvisor (custom node) → emits prompts, sampler settings, ControlNet/IPAdapter weights, upscale factor, and metadata
  • IPAdapterModelLoader → IPAdapterApply (weights driven by the advisor)
...
[Conditioning & Sampling]
  • CheckpointLoaderSimple (FLUX base/refiner) → IPAdapterApply → KSampler
  • CLIPTextEncode (positive/negative) driven by Ollama prompts
  • KSampler (receives IPAdapter-enhanced model, conditioning, latent encode, and advisor settings)
...
4. **Style Consistency via IPAdapter + Prompting**
   - The advisor reads both the source and reference images, summarises their content, and folds those descriptors into the positive prompt.
   - The generated `style_weight` simultaneously modulates the IPAdapter branch so that the reference palette and material cues carry through even when the prompt leans in a different direction.
...
- **IPAdapter**: an SDXL IPAdapter model plus matching CLIP-ViT-H vision weights (drop into `models/ipadapter/` and `models/clip_vision/`). Adjust the loader widget if you use different filenames.
